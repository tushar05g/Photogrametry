#!/usr/bin/env python3
"""
Test script to run the photogrammetry pipeline on the cube dataset.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(project_root / "logs" / "test_pipeline.log", mode='a')
    ]
)
...
def run_test():
    ...
    logger.info(f"🚀 Starting test pipeline run...")
    ...
    try:
        ...
        # Set workspace to a specific test folder for easier cleanup
        pipeline.workspace = os.path.join(output_dir, "test_workspace")
        if os.path.exists(pipeline.workspace):
            shutil.rmtree(pipeline.workspace)
        os.makedirs(pipeline.workspace, exist_ok=True)
        
        result = pipeline.run_pipeline(image_paths, "CubeTest")
